...
- 2026-08-19: `parseFood` returned `unknown` from the first build and nothing ever rendered it, so "250g of mince, a veetee pot, mixed veg" priced the mince and silently binned the other two -> the preview names what it could not place, in Joe's own words, read out of the residue so a partly-matched phrase reports only the missed part.
- 2026-08-19: An unrecognised phrase is offered stocked foods sharing a whole word (`suggestFoods`), never resolved to one — "a veetee pot" matches both Veetee pots and does not say which.
- 2026-08-19: "egg" dead-ended on an example sentence that did not mention it -> the panel carries "Scan its barcode" and "Type its macros"; a refusal needs a way out. Eggs are now stocked (Sainsbury's medium and large publish identical figures; their 0.5g fibre is a transcription error, recorded as 0).
- 2026-08-19: Adding "grilled" to FILLER stopped it being reported as an unknown food and left "200g grilled chicken" logging one 64g thigh — the weight scan could not cross it either. Preparation words belong in `BASIS` with cooked/raw/dry, so the number reaches the food and the word is read as meaning cooked. **Ignoring a word is not understanding it.**
