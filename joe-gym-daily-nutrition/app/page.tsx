      ...
      <section className="entry-card" id="meal-entry">
        ...
        {previewItems.length > 0 && (
          <div className="preview-panel">
            <div className="preview-foods">{previewItems.map((item) => <span key={item.id} className={item.assumed ? "assumed" : ""}>{item.name}<strong>{item.display}</strong>{item.assumed && <em>assumed — say the amount</em>}{item.weighedGrams !== undefined && <em className="note">from {round(item.weighedGrams)}g {item.weighedAs}</em>}</span>)}</div>
